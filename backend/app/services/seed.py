...
async def seed_database(db: AsyncSession) -> None:
    ...
    cpses = [CPSE(name=n, short_code=c, description="Synthetic demonstration organization") for n,c in [("Chennai Petroleum Corporation Limited","CPCL"),("Indian Oil Corporation Limited","IOCL"),("Steel Authority of India Limited","SAIL")]]
    categories = [MaterialCategory(name=n, code=c) for n,c in [("Valves","VLV"),("Pipes","PIP"),("Pumps","PMP"),("Motors","MTR"),("Bearings","BRG"),("Electrical Components","ELC")]]
    db.add_all(cpses + categories); await db.flush()
    db.add_all([User(name="BMIM Administrator", email="admin@example.com", password_hash=get_password_hash("Admin@123"), role=UserRole.ADMIN), User(name="Technical Reviewer", email="reviewer@example.com", password_hash=get_password_hash("Reviewer@123"), role=UserRole.TECHNICAL_REVIEWER), User(name="CPSE Manager", email="manager@example.com", password_hash=get_password_hash("Manager@123"), role=UserRole.CPSE_MANAGER, cpse_id=cpses[0].id)])
